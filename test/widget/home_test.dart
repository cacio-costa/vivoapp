...
void main() {
  ...
  testWidgets('Deve exibir saudação para usuário logado', (WidgetTester tester) async {
    await tester.pumpWidget(app!);

    await usuarioProvider!.login(
      Usuario(id: '1', nome: 'Fulano', email: 'fulano@email.com'),
    );
    await tester.pump();
  ...
  testWidgets('Deve exibir saudação para usuário logado', (WidgetTester tester) async {
    ...
    expect(find.textContaining('Você tem faturas prontas para pagar. Quer pagar agora?'), findsOneWidget);

    var btnOcultarNotificacao = find.byType(TextButton);
    await tester.tap(btnOcultarNotificacao);
    await tester.pump(const Duration(milliseconds: 1000));

    expect(find.textContaining('Você tem faturas prontas para pagar. Quer pagar agora?'), findsNothing);
  });
}
